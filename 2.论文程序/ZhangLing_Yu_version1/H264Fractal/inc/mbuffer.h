...
/////////////////////////////////////>
  int64       ref_pic_num_dec        [MAX_NUM_SLICES][6][MAX_LIST_SIZE];
  int64       frm_ref_pic_num_dec    [MAX_NUM_SLICES][6][MAX_LIST_SIZE];
  int64       top_ref_pic_num_dec    [MAX_NUM_SLICES][6][MAX_LIST_SIZE];
  int64       bottom_ref_pic_num_dec [MAX_NUM_SLICES][6][MAX_LIST_SIZE];
...
/////////////////////////////////////<
  ...
  int64 ***    ref_pic_id;    //!在P帧里存储scale和offset< reference picture identifier [list][subblock_x][subblock_y]
                             //   (not  simply index) 
  int64 ***    pre_ref_pic_id; //!在P帧里存储前一个图像的scale和offset< reference picture identifier [list][subblock_x][subblock_y]
  ...
  int  ****   pre_mv;        //!< motion vector  用于分形参数的预测编码
  byte **     moving_block;
...
// void             alloc_ref_pic_list_reordering_buffer(Slice *currSlice);++++++++++++Z
// void             free_ref_pic_list_reordering_buffer(Slice *currSlice);++++++++++++Z
...
void             store_picture_in_dpb_dec(StorablePicture* p);//编码解码可合并
void             fill_frame_num_gap_dec(ImageParameters *img);//编码解码可合并
void             dpb_split_field_dec(FrameStore *fs);//编码解码可合并
void             flush_dpb_dec();
