...
int start_sequence()
{
  ...
  switch(input->of_mode)//0
  {
    case PAR_OF_ANNEXB://0
      OpenAnnexbFile(input->outfile);//¼ì²é´ò¿ª
    ...
    case PAR_OF_RTP:
      ...
      break;
    default:
      printf(errortext, ET_SIZE, "Output File Mode %d not supported", input->of_mode);
      error(errortext,1);
      return 1;
  }
  ...
  len += WriteNALU (nalu);//104
  FreeNALU (nalu);
  nalu = NULL;
  nalu = GeneratePic_parameter_set_NALU ();
  len += WriteNALU (nalu);//176
  FreeNALU (nalu);

//  stat->bit_ctr_parametersets = len;
    stat->bit_ctr_parametersets_n = len;//176
  return 0;
}
