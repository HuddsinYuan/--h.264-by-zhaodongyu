...
static void output_one_frame_from_dpb_dec();
...
static void insert_picture_in_dpb_dec(FrameStore* fs, StorablePicture* p);
...
/////////////////////////////////////<
  ...
  get_mem3Dint64 (&(s->ref_pic_id), 6, (size_x / BLOCK_SIZE)*3/2, size_y / BLOCK_SIZE);//在P帧时存放scale和offset，第一维有6个，已经足够放YUV的scale和offset
  get_mem3Dint64 (&(s->pre_ref_pic_id), 6, (size_x / BLOCK_SIZE)*3/2, size_y / BLOCK_SIZE);//在P帧时存放前一帧的scale和offset
                                                                                     //在
  get_mem3Dint64 (&(s->ref_id), 6, size_x / BLOCK_SIZE, size_y / BLOCK_SIZE);
  get_mem4Dint (&(s->mv), 2, (size_x / BLOCK_SIZE)*3/2, size_y / BLOCK_SIZE,2 );//改变了大小
...
void store_picture_in_dpb(StorablePicture* p)
{
  ...
  {
    sliding_window_memory_management(p);//编码中的
  } 
...
//   if (!is_used_for_reference(dpb.fs[pos]))ZZZZZZZZZZZZZZZZZ因为会删除imgY_ref里的内容
//   {
...
//store reference picture indexZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZzz
//   if (!active_sps->frame_mbs_only_flag)
//   {
  ...
  for (i=0 ; i<fs->frame->size_x/4 ; i++)
  {
    for (j=0 ; j<fs->frame->size_y/8 ; j++)
    {
      ...
      if (!fs->frame->MbaffFrameFlag  || !fs->frame->mb_field[currentmb])    
      {
        ...
        // Scaling of references is done here since it will not affect spatial direct (2*0 =0)ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ
...
ColocatedParams* alloc_colocated(int size_x, int size_y, int mb_adaptive_frame_field_flag)
{
  ...
  get_mem3Dint (&(s->ref_idx), 2, size_x / BLOCK_SIZE, size_y / BLOCK_SIZE);
  get_mem3Dint64 (&(s->ref_pic_id), 2, (size_x / BLOCK_SIZE)*3/2, size_y / BLOCK_SIZE);
...
************************************************************************
*/
static void idr_memory_management_dec(StorablePicture* p)
{
	unsigned i;
...
************************************************************************
*/
static void sliding_window_memory_management_dec(StorablePicture* p)
{
	unsigned i;
 ...
 ************************************************************************
 */
void store_picture_in_dpb_dec(StorablePicture* p)
{
  unsigned i;
  int poc, pos;
  // diagnostics
      ...
      if ((int)dpb.last_picture->frame_num == p->pic_num)
      {
        if (((p->structure==TOP_FIELD)&&(dpb.last_picture->is_used==2))||((p->structure==BOTTOM_FIELD)&&(dpb.last_picture->is_used==1)))
        {
          if ((p->used_for_reference && (dpb.last_picture->is_orig_reference!=0))||
              (!p->used_for_reference && (dpb.last_picture->is_orig_reference==0)))
          {
            insert_picture_in_dpb_dec(dpb.last_picture, p);
            update_ref_list();
            update_ltref_list();
            dump_dpb();
            dpb.last_picture = NULL;
  ...
  {
    sliding_window_memory_management_dec(p);//解码中的 出问题了
  } 
  ...
  // store at end of buffer
//  printf ("store frame/field at pos %d\n",dpb.used_size);
  insert_picture_in_dpb_dec(dpb.fs[dpb.used_size],p);
 ...
 ************************************************************************
 */
void fill_frame_num_gap_dec(ImageParameters *img)
{
  int CurrFrameNum;
  int UnusedShortTermFrameNum;
  StorablePicture *picture = NULL;
...
//  printf("A gap in frame number is found, try to fill it.\n");
  ...
  while (CurrFrameNum != UnusedShortTermFrameNum)
  {
    ...
    store_picture_in_dpb_dec(picture);
...
//  printf ("insert (%s) pic with frame_num #%d, poc %d\n", (p->structure == FRAME)?"FRAME":(p->structure == TOP_FIELD)?"TOP_FIELD":"BOTTOM_FIELD", p->pic_num, p->poc);
  ...
  if (fs->is_used==3)
  {
    if (p_ref)
		;
       find_snr_dec(snr, fs->frame, p_ref);//ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ
  }
}
 ...
 ************************************************************************
 */
void dpb_split_field_dec(FrameStore *fs)
{
  int i,j;
  int dummylist0,dummylist1;
  ...
  //store reference picture indexZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZz
