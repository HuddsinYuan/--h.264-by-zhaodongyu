...
//   printf("RBSPtoNALU\n");
  ...
  nalu->nal_reference_idc = nal_reference_idc;//3
  nalu->nal_unit_type = nal_unit_type;//7
  nalu->startcodeprefix_len = UseAnnexbLongStartcode?4:3;//4
  nalu->buf[0] =//103
    nalu->forbidden_bit << 7      |//0
    nalu->nal_reference_idc << 5  |//3
    nalu->nal_unit_type;//7
...
// printf ("RBSPtoNALU: Before: NALU len %d\t RBSP %x %x %x %x\n", rbsp_size, (unsigned) nalu->buf[1], (unsigned) nalu->buf[2], (unsigned) nalu->buf[3], (unsigned) nalu->buf[4]);
  ...
  len = 1 + RBSPtoEBSP (&nalu->buf[1], 0, rbsp_size, min_num_bytes);//9
