...
/* Range table for  LPS */ 
extern byte rLPS_table_64x4[64][4];
...
extern unsigned short AC_next_state_MPS_64[64];
...
// };
// 
extern unsigned short AC_next_state_LPS_64[64];
...
void arideco_delete_decoding_environment(DecodingEnvironmentPtr dep)
{
  if (dep == NULL)
  {
    printf(errortext, ET_SIZE, "Error freeing dep (NULL pointer)");
    error (errortext, 200);
  }
  else
    free(dep);
                            ...
                            int firstbyte, int *cpixcode_len, int slice_type )
{
  ...
  *Dcodestrm_len = firstbyte;
  ...
  dep->Drange = HALF-2;//HALF=(1 << (B_BITS-1))//B_BITS=10
  dep->Dvalue = value;
}
...
{
  register unsigned int bit = bi_ct->MPS;//0
  register unsigned int value = dep->Dvalue;//172
  register unsigned int range = dep->Drange;//498
  register unsigned int rLPS = rLPS_table_64x4[bi_ct->state][(range>>6) & 0x03];//150
 ...
 ************************************************************************
 */
void biari_init_context_dec (struct img_par* img, BiContextTypePtr ctx, const int* ini)
{
  int pstate;

  pstate = ((ini[0]*img->qp)>>4) + ini[1];
//   printf("ini[0]=%d,ini[1]=%d    ",ini[0],ini[1]);
