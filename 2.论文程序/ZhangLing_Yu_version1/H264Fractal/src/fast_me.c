...
void DefineThresholdMB()
{
  ...
  int gb_q_bits    = Q_BITS+gb_qp_per;//Q_BITS=16
  ...
  Bsize[7]=(16*16)*Quantize_step;//Bsize[7]是4*4块

  Bsize[6]=Bsize[7]*4;//Bsize[6]是8*8块
  Bsize[5]=Bsize[7]*4;
  Bsize[4]=Bsize[5]*4;
  Bsize[3]=Bsize[4]*4;
  Bsize[2]=Bsize[4]*4;
...
int get_mem_mincost (int****** mv)
{
  ...
  if ((*mv = (int*****)calloc(input->imagewidth/4,sizeof(int****))) == NULL)
    no_mem_exit ("get_mem_mv: mv");
  for (i=0; i<input->imagewidth/4; i++)
  {
    if (((*mv)[i] = (int****)calloc(input->imageheight/4,sizeof(int***))) == NULL)
      no_mem_exit ("get_mem_mv: mv");
    for (j=0; j<input->imageheight/4; j++)
    {
  ...
  return input->imagewidth/4*input->imageheight/4*img->max_num_references*9*3*sizeof(int);
}
/*!
 *******************************************************************************
 * \brief
...
FastIntegerPelBlockMotionSearch  (pel_t**   orig_pic,     // <--  not used
                  int       ref,          // <--  reference frame (0... or -1 (backward))
                  int       list,
				  int       pic_pix_x,    // <--  absolute x-coordinate of regarded AxB block像素在图像中的X坐标

				  int       pic_pix_y,    // <--  absolute y-coordinate of regarded AxB block像素在图像中的Y坐标
                  ...
                  double    lambda)       // <--  lagrangian parameter for determining motion cost
{
	static int Diamond_x[4] = {-1, 0, 1, 0};//菱形4个点 与中心点距离都是1

  static int Diamond_y[4] = {0, 1, 0, -1};
  static int Hexagon_x[6] = {2, 1, -1, -2, -1, 1};//6边形
  ...
  pel_t *(*get_ref_line)(int, pel_t*, int, int, int, int);//get_ref_line函数指针
//   如果图像是桢场自适应模式,且宏块场编码,顶是2 低是4
// 	  只是图像场编码或者桢编码为0

  int   list_offset   = ((img->MbaffFrameFlag)&&(img->mb_data[img->current_mb_nr].mb_field))? img->current_mb_nr%2 ? 4 : 2 : 0;
  //list+listoffset决定采用那个列表 ,列表中参考索引ref
  pel_t*  ref_pic       = listX[list+list_offset][ref]->imgY_11;//参考桢亮度有边界填充img->type==B_IMG? Refbuf11 [ref+((mref==mref_fld)) +1] : Refbuf11[ref];
  int   best_pos      = 0;                                        // position with minimum motion cost

  int   max_pos       = (2*search_range+1)*(2*search_range+1);    // number of search positions全搜索Fs最大搜索位置数目
  int   lambda_factor = LAMBDA_FACTOR (lambda);                   // factor for determining lagragian motion cost计算lambda因子

  int   mvshift       = 2;                  // motion vector shift for getting sub-pel units   mv向量移位为了子像素单元

  int   blocksize_y   = input->blc_size[blocktype][1];            // vertical block size块垂直尺寸

  int   blocksize_x   = input->blc_size[blocktype][0];            // horizontal block size
  int   blocksize_x4  = blocksize_x >> 2;                         // horizontal block size in 4-pel units4像素做单元的块水平尺寸
  
  // pred_x预测位置 1/4像素单位预测mv矢量(mvshift=2)
  ...
  int    best_x, best_y;
  int   check_for_00  = (blocktype==1 && !input->rdopt && img->type!=B_SLICE && ref==0);//check_for_00  表示16*16 不用率失真优化模式
  int   search_step,iYMinNow, iXMinNow;
  int   i,m, iSADLayer; 
  int   iAbort;
  int       N_Bframe = input->successive_Bframe;
  float betaSec,betaThird;
  int height=((img->MbaffFrameFlag)&&(img->mb_data[img->current_mb_nr].mb_field))?img->height/2:img->height;//在图象自适应桢场模式且宏块场编码模式下 高度除2
  ...
  {//search_range
// 	  当前块的左上在搜索范围内移动在图像width*height面积内 ref_line是函数指针指向FastLineX指针函数地址,FastLineX指针函数返回的是byte指针

    get_ref_line = FastLineX;
  }
  else//当前块的左上在搜索范围内移动在不图像width*height范围内
  ...
  if ((img->current_frame > 1 + ref && ref!=-1) || (list == 1 && (Bframe_ctr%N_Bframe) > 1))  //for debug
  {
