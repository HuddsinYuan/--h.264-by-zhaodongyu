 ...
 ************************************************************************
 *  \brief
 *     This function generates the slice (and partition) header(s) 生成条带头
 *
 *  \return number of bits used for the slice (and partition) header(s)返回条带头需要的比特数
 *
 *  \par Side effects:
...
int start_slice()
{
  ...
  if(input->symbol_mode == CABAC)//1
  {
    init_contexts();//初始化上下文模型
  }
  return header_len;//32
}
...
int terminate_slice()
{
  ...
  if (input->symbol_mode == CABAC)
    write_terminating_bit (1);      // 对片结束符号1进行编码only once, not for all partitions
  ...
  for (i=0; i<currSlice->max_part_nr; i++)
  {
    ...
    else     // CABAC
    {
      ...
      arienco_done_encoding(eep);//结束算术编码码字
 ...
 ************************************************************************
 * \brief
 *    Encodes one slice//重要的编码一个条带的函数！！！！
 * \par
...
int encode_one_slice (int SliceGroupId, Picture *pic)
{
  ...
  CurrentMbAddr = FmoGetFirstMacroblockInSlice (SliceGroupId);//计算条带中的第一个宏块地址
// printf ("\n\nEncode_one_slice: PictureID %d SliceGroupId %d  SliceID %d  FirstMB %d \n", img->tr, SliceGroupId, img->current_slice_nr, CurrentMbInScanOrder);
  ...
  init_slice (CurrentMbAddr);////////////////////////////为当前要编码的slice申请一个Slice类型结构体，并进行初始化
  Bytes_After_Header = img->currentSlice->partArr[0].bitstream->byte_pos;

  if (input->symbol_mode==CABAC)
  {
    SetCtxModelNumber ();//设置上下文模型
  }
...
*/
  ...
  len = start_slice ();  //25    /////////////////////////写slice_header

  // Rate control
  img->NumberofHeaderBits +=len;//计算条带头比特数25
...
*/
  // Update statistics
  stat->bit_slice += len;//0+25
  stat->bit_use_header[img->type] += len;//0+25
// printf ("\n\n");

  while (end_of_slice == FALSE1) // loop over macroblocks 对每个slice进行编码，用帧或场方式
  {
    //sw paff
    if (!img->MbaffFrameFlag)//yes
    {
      recode_macroblock = FALSE1;
      rdopt = &rddata_top_frame_mb;   // store data in top frame MB 
      
      start_macroblock (CurrentMbAddr, FALSE1);//初始化宏块

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      i_encode_one_macroblock ();////////////////////////////////////////////////////////////////////////////////////////////重要的编码一个I帧宏块的函数,选择最佳帧内编码编码模式
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//!      already generated bitstream) for the "test coding"?  
      ...
      if (input->MbInterlace == ADAPTIVE_CODING)//no
      {
        ...
        rdopt = &rddata_top_frame_mb; // store data in top frame MB 
        i_encode_one_macroblock ();     // code the MB as frame
        ...
        start_macroblock (CurrentMbAddr+1, FALSE1);
      ...
      if ((input->MbInterlace == ADAPTIVE_CODING) || (input->MbInterlace == FIELD_CODING))//no
      {
        //Rate control
        img->bot_MB = 0; 
        ...
        img->num_ref_idx_l0_active += 1;
        start_macroblock (CurrentMbAddr, TRUE1);
...
//        TopFieldIsSkipped = 0;        // set the top field MB skipped flag to 0
        ...
        img->top_field = 0;   // Set top field to 0
        start_macroblock (CurrentMbAddr+1, TRUE1);
      ...
      if ((input->MbInterlace == ADAPTIVE_CODING) && (FrameRDCost < FieldRDCost))//no
      {
        img->field_mode = 0;
        img->buf_cycle >>= 1;
        input->num_reference_frames >>= 1;
      ...
      if (CurrentMbAddr == FmoGetLastCodedMBOfSliceGroup (FmoMB2SliceGroup (CurrentMbAddr)))
        end_of_slice = TRUE1;        // just in case it does n't get set in terminate_macroblock  
    }
  }  
/*
  // Tian Dong: June 7, 2002 JVT-B042
