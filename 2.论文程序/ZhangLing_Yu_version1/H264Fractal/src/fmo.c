...
int FmoInit(ImageParameters * img, pic_parameter_set_rbsp_t * pps, seq_parameter_set_rbsp_t * sps)
{
  ...
  FmoGenerateMapUnitToSliceGroupMap(img, pps);///////////主要根据slice_group_map_type这个变量决定片到片组的映射关系，既确定那些片属于同一个片组。
  FmoGenerateMBAmap(img, sps);////////////////////////////确定宏块到片的映射，既那些宏块属于一个片。
  
#ifdef PRINT_FMO_MAPS//没运行
  printf("\n");
...
int FmoStartPicture ()
{
  ...
  for (i=0; i<MAXSLICEGROUPIDS; i++)
    FirstMBInSlice[i] = FmoGetFirstMBOfSliceGroup (i);//里面有东西
  return 0;
}
...
{
  int i = 0;
  while ((i<(int)img->PicSizeInMbs) && (FmoMB2SliceGroup (i) != SliceGroupID))//里面有东西
    i++;
...
int FmoInit_dec(pic_parameter_set_rbsp_t* pps, seq_parameter_set_rbsp_t* sps)
{
	...
	FmoGenerateMapUnitToSliceGroupMap(img, pps);//此处应该为解码的函数，但和编码的内容一样，故改变了参数
