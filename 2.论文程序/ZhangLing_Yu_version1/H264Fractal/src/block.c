...
//先讨论下quant_coef如何得到地：(见相关文档)量化系数矩阵
...
void intrapred_luma(int img_x,int img_y, int *left_available, int *up_available, int *all_available)
{
  ...
  *all_available  = block_available_up && block_available_left && block_available_up_left;
  ...
  for (i=0; i < BLOCK_SIZE; i++)
  {
    img->mprr[VERT_PRED][0][i] = //all 9 prediction modes? 
		                         // enlarged from 4 to 16 for ABT (is that neccessary?)
...
void intrapred_luma_16x16()
{
  ...
  if (!(input->UseConstrainedIntraPred))//1
  {
    up_avail   = up.available;//0
    left_avail = left[1].available;//0
    left_up_avail = left[0].available;//0
  }
  else
  {
...
int dct_luma_16x16(int new_intra_mode)
{
  ...
  Macroblock *currMB = &img->mb_data[img->current_mb_nr];// Macroblock    *mb_data; /current_mb_nr当前宏块索引号
  ...
  qp_per    = (currMB->qp-MIN_QP)/6;//MIN_QP=0
  qp_rem    = (currMB->qp-MIN_QP)%6;//求出当前块量化索引在表中的行值：quant_coef[qp_rem]
  q_bits    = Q_BITS+qp_per;//Q_BITS=15
  qp_const  = (1<<q_bits)/3;//  1<<x 相当于2的x次方，帧内模式除以3，帧间模式除以6

  for (j=0;j<16;j++)
  {
    for (i=0;i<16;i++)
    {//imgY_org++ 待编码原始图像的亮度像素值  if (mode == I16MB) 用的是 mprr_2，否则就用 mpr
      //M1存放残16*16残差值，mprr_2[5][16][16]存放预测值
		M1[i][j]=imgY_org[img->opix_y+j][img->opix_x+i]-img->mprr_2[new_intra_mode][j][i];//!< img->opix_x current original picture pixel horizontal
// 		printf("(%d,%d)  ",imgY_org[img->opix_y+j][img->opix_x+i],img->mprr_2[new_intra_mode][j][i]);
        M0[i%4][i/4][j%4][j/4]=M1[i][j];
    }
  }
/*参考毕厚杰p106，下面程序采用快速变换算法:先对残差块行之间进行一维整数DCT,然后对经行变换的块的列之间进行一维整数DCT*/
  for (jj=0;jj<4;jj++)
  {
    for (ii=0;ii<4;ii++)
    {
      ...
      for (i=0;i<4;i++)
      {
        ...
        M0[i][ii][3][jj]=M5[3] -M5[2]*2;//这时M0即为书中的W
      }
    }
  }

  // pick out DC coeff,并对4*4 DC系数进行哈达玛变换，变换过程与快速DCT类似，个别参数有变化
  ...
  for (i=0;i<4;i++)
  {
    ...
    M4[i][0]=(M5[0]+M5[1])>>1;//根据哈达玛变换定义矩阵运算后除以2
    M4[i][2]=(M5[0]-M5[1])>>1;
    M4[i][1]=(M5[3]+M5[2])>>1;
    M4[i][3]=(M5[3]-M5[2])>>1;
  }
  ...
  for (coeff_ctr=0;coeff_ctr<16;coeff_ctr++)
  {
    ...
    //对DC系数量化，quant_coef即Iain书上表6.6的MF
    ...
    if (level != 0)
    {
      ...
      DCRun  [scan_pos] = run;
// 	  printf("DCLevel[%d]=%d,DCRun[%d]=%d;\n",scan_pos,DCLevel[scan_pos],scan_pos,DCRun[scan_pos]);
      ++scan_pos;
      run=-1;
    }
    M4[i][j]=sign(level,M4[i][j]);//使量化后的值与原来DCT系数符号相同
  }
  DCLevel[scan_pos]=0;

  // invers DC transform，和之前的哈达玛变化基本类似，最终将反变换及去量化的值存入MO相应位置
  for (j=0;j<4;j++)
  {
    for (i=0;i<4;i++)
      M5[i]=M4[i][j];
  ...
  for (jj=0;jj<4;jj++)
  {
    for (ii=0;ii<4;ii++)
    {
      ...
      for (coeff_ctr=1;coeff_ctr<16;coeff_ctr++) // set in AC coeff
      {
        ...
        M0[i][ii][j][jj]=sign(level*dequant_coef[qp_rem][i][j]<<qp_per,M0[i][ii][j][jj]);//将去量化的DCT系数存入MO中
      }
      ACLevel[scan_pos] = 0;
  ...
  for (j=0;j<16;j++)
    for (i=0;i<16;i++)
	{//将帧内16*16宏块的重建数据写入enc_picture->imgY[img->pix_y+j][img->pix_x+i]
		//#define DQ_ROUND  (1<<(DQ_BITS-1)),Q_BITS=6
...
// 		printf("(%d;%d;%d)  ",img->mprr_2[new_intra_mode][j][i],imgY_org[img->opix_y+j][img->opix_x+i],enc_picture->imgY[img->pix_y+j][img->pix_x+i]);
	}
    return ac_coef;
}

//
/*!//将4*4块残差经过DCT、量化、反量化、IDCT后的值存入enc_picture->imgY,返回非零系数的个数
 ************************************************************************
 * \brief
 ...
 ************************************************************************
 */
int dct_luma(int block_x,int block_y,int *coeff_cost, int old_intra_mode)//参数block_x, block_y表示当前4*4块在当前宏块中的位置（0，4，8，12）
{
  ...
  int ACLevel_N=0,ACRun_N=0;

  int   pos_x   = block_x/BLOCK_SIZE;// BLOCK_SIZE=4， pos_x 表示是宏块中的第几个4*4块
  int   pos_y   = block_y/BLOCK_SIZE;
  int   b8      = 2*(pos_y/2) + (pos_x/2);//代表宏块中第几个8*8块吧？？（0，1，2，3）
  int   b4      = 2*(pos_y%2) + (pos_x%2);//代表所在8*8块中第几个4*4块吧？？（0，1，2，3）
  int*  ACLevel = img->cofAC[b8][b4][0];// img->cofAC代表什么含义？？？是在哪里进行赋值的呢？？？
  int*  ACRun   = img->cofAC[b8][b4][1];// img->cofAC代表什么含义？？？是在哪里进行赋值的呢？？？

  Macroblock *currMB = &img->mb_data[img->current_mb_nr];
//   printf("dct_luma");

  qp_per    = (currMB->qp-MIN_QP)/6;//4
  qp_rem    = (currMB->qp-MIN_QP)%6;//4
  q_bits    = Q_BITS+qp_per;//19

  if (img->type == I_SLICE)
    qp_const=(1<<q_bits)/3;    // intra //87381
  else
    qp_const=(1<<q_bits)/3;    // inter //174762

  //  Horizontal transform水平变换
  for (j=0; j < BLOCK_SIZE; j++)//和左边矩阵相乘
  {
    for (i=0; i < 2; i++)
    {
      i1=3-i;
  ...
  //  Vertical transform垂直变换
  for (i=0; i < BLOCK_SIZE; i++)//和右边矩阵相乘
  {
    for (j=0; j < 2; j++)
    {
      j1=3-j;
  ...
  for (coeff_ctr=0;coeff_ctr < 16;coeff_ctr++)//一个4*4块
  {

    if (img->field_picture || ( img->MbaffFrameFlag && currMB->mb_field )) //no
    {  // Alternate scan for field coding
        i=FIELD_SCAN[coeff_ctr][0];
        j=FIELD_SCAN[coeff_ctr][1];
    }
    ...
    {//（i,j）是Z字型扫描的顺序
        i=SNGL_SCAN[coeff_ctr][0];//0,1,0,0,1,2,3,2,1,0,1,2,3,3,2,3
        j=SNGL_SCAN[coeff_ctr][1];//0,0,1,2,1,0,0,1,2,3,3,2,1,2,3,3
    }
    
    run++;//run代表的是最后一个非零系数之后零的个数
    ilev=0;
    //对变换后的残差进行量化
    level = (abs (img->m7[i][j]) * quant_coef[qp_rem][i][j] + qp_const) >> q_bits;    //正向量化，Zij=(|Wij|*MF+f)>>qbits
    
    if (level != 0)//量化后的值不等于0
    {
      nonzero=TRUE;
      if (level > 1)//如果量化后的值大于1，则不能被截断
        *coeff_cost += MAX_VALUE;                // set high cost, shall not be discarded
      else//量化后的值小于等于1
        *coeff_cost += COEFF_COST[run];//COEFF_COST根据run的不同值分别为3，2，1，0
      ACLevel[scan_pos] = sign(level,img->m7[i][j]);//带有符号的量化值
      ACRun  [scan_pos] = run;
      ++scan_pos;// //遇到非零系数就加1
      run=-1;// reset zero level counter         //遇到非零系数就重置run为-1
      ilev=level*dequant_coef[qp_rem][i][j]<<qp_per;//对量化后的值进行反量化,
    }
    img->m7[i][j]=sign(ilev,img->m7[i][j]); //现在img->m7[i][j]存储的是残差经DCT、量化、反量化后的值了  //img->m7[i][j]>0,则返回ilev的绝对值，否则返回相反数
  }
  ACLevel[scan_pos] = 0;//现在scan_pos的值等于非零系数的个数
  
  //以上返回的img->m7[i][j]就是量化反量化后的结果
...
//////////////////
	...
	{
      j1=3-j;
      img->m7[i][j] =min(255,max(0,(m6[j]+m6[j1]+(img->mpr[i+block_x][j+block_y] <<DQ_BITS)+DQ_ROUND)>>DQ_BITS));//DQ_BITS=6
      img->m7[i][j1]=min(255,max(0,(m6[j]-m6[j1]+(img->mpr[i+block_x][j1+block_y]<<DQ_BITS)+DQ_ROUND)>>DQ_BITS));//DQ_ROUND=(1<<(DQ_BITS-1))
	}
// 	for (j=0; j < BLOCK_SIZE; j++)
// 	{
// 		printf("\n");
// 		for (i=0; i < BLOCK_SIZE; i++)
// 			printf("mpr=%3d ",img->mpr[i+block_x][j+block_y]);
// 	}

  }//以上得到的 img->m7[i][j] 就是4*4块残差经过DCT、量化、反量化、IDCT后的值
 
  //  Decoded block moved to frame memory
  if (0==UV_dct&&img->type==0)//Y分量
  { 
//   	printf("\nenc_pic");
  	for (j=0; j < BLOCK_SIZE; j++)
	{	
// 		printf("\n");
		for (i=0; i < BLOCK_SIZE; i++)
		{	
			enc_picture->imgY[img->pix_y+block_y+j][img->pix_x+block_x+i] = img->m7[i][j];//将4*4块残差经过DCT、量化、反量化、IDCT后的值存入enc_picture->imgY

// 			printf("%3d ",enc_picture->imgY[img->pix_y+block_y+j][img->pix_x+block_x+i]);
		}
	}
  } 
  else if(1==UV_dct)//U分量
  {
	for (j=0; j < BLOCK_SIZE; j++)
		for (i=0; i < BLOCK_SIZE; i++)
			enc_picture->imgUV[0][img->pix_y+block_y+j][img->pix_x+block_x+i]=img->m7[i][j];
  }
  else if(2==UV_dct)//V分量
  {
	for (j=0; j < BLOCK_SIZE; j++)
		for (i=0; i < BLOCK_SIZE; i++)
			enc_picture->imgUV[1][img->pix_y+block_y+j][img->pix_x+block_x+i]=img->m7[i][j];
...
//         printf("\nn1+i=%d,n2+j=%d,n2+j2=%d",n1+i,n2+j,n2+j2);
		}
      }
    }
  }
