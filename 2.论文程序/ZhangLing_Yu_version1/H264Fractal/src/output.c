...
void write_out_picture_dec(StorablePicture *p, FILE *p_out);

#ifdef PAIR_FIELDS_IN_OUTPUT

void clear_picture_dec(StorablePicture *p);
...
void flush_pending_output(FILE *p_out)
{
  ...
  {
    write_out_picture_dec(pending_output, p_out);
  }
 ...
 ************************************************************************
 */
void write_picture_dec(StorablePicture *p, FILE *p_out, int real_structure)
{
   int i, add;
  ...
  if (real_structure==FRAME)
  {
    ...
    write_out_picture_dec(p, p_out);
    return;
  }
  if (real_structure==pending_output_state)
  {
    flush_pending_output(p_out);
    write_picture_dec(p, p_out, real_structure);
    return;
  }
 ...
 ************************************************************************
 */
void write_unpaired_field_dec(FrameStore* fs, FILE *p_out)
{
  StorablePicture *p;
  assert (fs->is_used<3);
  if(fs->is_used &1)
  {
    ...
    clear_picture(fs->bottom_field);
    dpb_combine_field(fs);
    write_picture_dec (fs->frame, p_out, TOP_FIELD);
  }
  ...
  if(fs->is_used &2)
  {
    ...
    dpb_combine_field(fs);
    write_picture_dec (fs->frame, p_out, BOTTOM_FIELD);
  }
 ...
 ************************************************************************
 */
void flush_direct_output_dec(FILE *p_out)
{
  write_unpaired_field(out_buffer, p_out);
 ...
 ************************************************************************
 */
void write_stored_frame_dec( FrameStore *fs,FILE *p_out)
{
  // make sure no direct output field is pending
  flush_direct_output(p_out);
  ...
  {
    write_picture_dec(fs->frame, p_out, FRAME);
  }
 ...
 ************************************************************************
 */
void direct_output_dec(StorablePicture *p, FILE *p_out)
{
  if (p->structure==FRAME)
  {
    // we have a frame (or complementary field pair)
    // so output it directly
    flush_direct_output(p_out);
    write_picture_dec (p, p_out, FRAME);
     if (p_ref)//ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ
       find_snr_dec(snr, p, p_ref);//
    free_storable_picture(p);
    return;
  }
  ...
  if (out_buffer->is_used == 3)
  {
    // we have both fields, so output them
    dpb_combine_field(out_buffer);
    write_picture_dec (out_buffer->frame, p_out, FRAME);
     if (p_ref)//ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZz
       find_snr_dec(snr, out_buffer->frame, p_ref);//
    free_storable_picture(out_buffer->frame);
    out_buffer->frame = NULL;
    free_storable_picture(out_buffer->top_field);
    out_buffer->top_field = NULL;
